 ...
 */var GraphSpreading=function(){/**
   ...
   */},{key:'unspreadFromRemovedSpreadLinkByPrevId',value:function unspreadFromRemovedSpreadLinkByPrevId(spreadLinkId,context,handler,callback){var _this9=this;var query={prev:spreadLinkId};this._wrapSpreadQuery(query,context);if(handler){this.spreadGraph.fetch(query,undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this9.each(spreadLinks,function(spreadLink,next){_this9.spreadGraph.remove(spreadLink.id,function(error,count){handler(error,spreadLink);next()},context)},function(){if(callback)callback(undefined,spreadLinks.length)})}})}else{this.spreadGraph.remove(query,callback,context)}}/**
   ...
   */},{key:'unspreadByPathId',value:function unspreadByPathId(pathLinkId,context,handler,callback){var _this10=this;var query={path:pathLinkId};this._wrapSpreadQuery(query,context);if(handler){this.spreadGraph.fetch(query,undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this10.each(spreadLinks,function(spreadLink,next){_this10.spreadGraph.remove(spreadLink.id,function(error,count){handler(error,spreadLink);next()},context)},function(){if(callback)callback(undefined,spreadLinks.length)})}})}else{this.spreadGraph.remove(query,callback,context)}}/**
   * Optional handler.
   *
   ...
   */},{key:'unspread',value:function unspread(id,context,handler,callback){var _this11=this;var query=_defineProperty({},this.spreadGraph.variableField,id);this._wrapSpreadQuery(query,context);this.spreadGraph.fetch(query,undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this11.each(spreadLinks,function(spreadLink,next){_this11.spreadGraph._unspreadingHandler(spreadLink,context,function(permission){if(permission){_this11.spreadGraph.remove(spreadLink.id,function(error,count){if(handler)handler(error,spreadLink);next()},context)}})},function(){if(callback)callback(undefined,spreadLinks.length)})}})}/**
   * Optional handler.
   *
