 ...
 */var GraphSpreading=function(){/**
   ...
   * @param {GraphSpreading~spreadFromSpreadLinkByPathLinkCallback} [handler]
   * @param {GraphSpreading~spreadByPathLinkCallback} [callback]
   */},{key:'spreadByPathLink',value:function spreadByPathLink(pathGraph,pathLink,context,handler,callback){var _this2=this;var fromFields=context&&context.fromFields||pathGraph.fromFields;this.each(fromFields,function(fromField,next){_this2._spreadByPathLink(fromField,pathGraph,pathLink,context,handler,next)},callback)}/**
   * Optional callback.
   *
   ...
   * @param {GraphSpreading~spreadFromSpreadLinkByPathGraphHandler} [handler]
   * @param {GraphSpreading~spreadFromSpreadLinkByPathGraphCallback} [callback]
   */},{key:'spreadFromSpreadLinkByPathGraph',value:function spreadFromSpreadLinkByPathGraph(spreadLink,pathGraph,context,handler,callback){var _this6=this;this.each(pathGraph.fromFields,function(fromField,next){_this6._spreadFromSpreadLinkByPathGraph(fromField,spreadLink,pathGraph,context,handler,next)},callback)}/**
   ...
   */},{key:'_spreadFromSpreadLinkByPathLink',value:function _spreadFromSpreadLinkByPathLink(toField,spreadLink,pathGraph,pathLink,context,callback){var _spreadGraph$_spreadi,_this7=this;this.spreadGraph._spreadingHandler(spreadLink,pathGraph,pathLink,(_spreadGraph$_spreadi={},_defineProperty(_spreadGraph$_spreadi,this.spreadGraph.constantField,spreadLink[this.spreadGraph.constantField]),_defineProperty(_spreadGraph$_spreadi,this.spreadGraph.variableField,pathLink[toField]),_defineProperty(_spreadGraph$_spreadi,'prev',spreadLink.id),_defineProperty(_spreadGraph$_spreadi,'path',pathLink.id),_defineProperty(_spreadGraph$_spreadi,'root',spreadLink.root?spreadLink.root:spreadLink.id),_spreadGraph$_spreadi),context,function(newSpreadLink){if(newSpreadLink){_this7.spreadGraph.insert(newSpreadLink,function(error,id){if(callback)callback(error,id,spreadLink,pathGraph,pathLink)},context)}else{if(callback)callback()}})}/**
   ...
   */},{key:'unspreadFromRemovedSpreadLinkByPrevId',value:function unspreadFromRemovedSpreadLinkByPrevId(spreadLinkId,context,handler,callback){var _this9=this;if(handler){this.spreadGraph.fetch({prev:spreadLinkId},undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this9.each(spreadLinks,function(spreadLink,next){_this9.spreadGraph.remove(spreadLink.id,function(error,count){handler(error,spreadLink);next()},context)},function(){if(callback)callback(undefined,spreadLinks.length)})}})}else{this.spreadGraph.remove({prev:spreadLinkId},callback,context)}}/**
   ...
   */},{key:'unspreadByPathId',value:function unspreadByPathId(pathLinkId,context,handler,callback){var _this10=this;if(handler){this.spreadGraph.fetch({path:pathLinkId},undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this10.each(spreadLinks,function(spreadLink,next){_this10.spreadGraph.remove(spreadLink.id,function(error,count){handler(error,spreadLink);next()},context)},function(){if(callback)callback(undefined,spreadLinks.length)})}})}else{this.spreadGraph.remove({path:pathLinkId},callback,context)}}/**
   * Optional handler.
   *
   ...
   */},{key:'unspread',value:function unspread(id,context,handler,callback){var _this11=this;this.spreadGraph.fetch(_defineProperty({},this.spreadGraph.variableField,id),undefined,function(error,spreadLinks){if(error){if(callback)callback(error)}else{_this11.each(spreadLinks,function(spreadLink,next){_this11.spreadGraph._unspreadingHandler(spreadLink,context,function(permission){if(permission){_this11.spreadGraph.remove(spreadLink.id,function(error,count){if(handler)handler(error,spreadLink);next()},context)}})},function(){if(callback)callback(undefined,spreadLinks.length)})}})}/**
   * Optional handler.
   *
   ...
   */},{key:'spreadTo',value:function spreadTo(id,context,handler,callback){var _this12=this;this.each(this.pathGraphs,function(pathGraph,nextPathGraph){var toFields=context&&context.toFields||pathGraph.toFields;_this12.each(toFields,function(toField,nextToField){pathGraph.fetch(_defineProperty({},toField,id),undefined,function(error,pathLinks){_this12.each(pathLinks,function(pathLink,nextPathLink){_this12.spreadByPathLink(pathGraph,pathLink,context,handler,nextPathLink)},function(error){nextToField()})})},nextPathGraph)},function(){if(callback)callback()})}/**
