...
var GraphSpreading = function () {
  ...
  _createClass(GraphSpreading, [{
    ...
    value: function spreadByPathLink(pathGraph, pathLink, context, handler, callback) {
      ...
      this.spreadGraph.fetch(_defineProperty({}, this.spreadGraph.variableField, pathLink[pathGraph.fromField]), undefined, function (error, spreadLinks) {
    ...
    value: function spreadNewSpreadLink(newSpreadLink, context, callback) {
      ...
      this.spreadGraph._spreadingHandler(undefined, undefined, undefined, newSpreadLink, context, function (newSpreadLink) {
        if (newSpreadLink) {
          _this2.spreadGraph.insert(newSpreadLink, callback, context);
        } else {
          if (callback) callback();
        }
      });
    ...
    value: function spreadFromSpreadLinkByPathGraph(spreadLink, pathGraph, context, handler, callback) {
      ...
      pathGraph.fetch(_defineProperty({}, pathGraph.fromField, spreadLink[this.spreadGraph.variableField]), undefined, function (error, pathLinks) {
    ...
    value: function spreadFromSpreadLinkByPathLink(spreadLink, pathGraph, pathLink, context, callback) {
      ...
      this.spreadGraph._spreadingHandler(spreadLink, pathGraph, pathLink, (_spreadGraph$_spreadi = {}, _defineProperty(_spreadGraph$_spreadi, this.spreadGraph.constantField, spreadLink[this.spreadGraph.constantField]), _defineProperty(_spreadGraph$_spreadi, this.spreadGraph.variableField, pathLink[pathGraph.toField]), _defineProperty(_spreadGraph$_spreadi, 'prev', spreadLink.id), _defineProperty(_spreadGraph$_spreadi, 'path', pathLink.id), _defineProperty(_spreadGraph$_spreadi, 'root', spreadLink.root ? spreadLink.root : spreadLink.id), _spreadGraph$_spreadi), context, function (newSpreadLink) {
        if (newSpreadLink) {
          _this5.spreadGraph.insert(newSpreadLink, function (error, id) {
    ...
    value: function unspread(id, context, handler, callback) {
      ...
      this.spreadGraph.fetch(_defineProperty({}, this.variableField, id), undefined, function (error, spreadLinks) {
        if (error) {
          if (callback) callback(error);
        } else {
          var queue = _async2.default.queue(function (spreadLink, next) {
            _this8.spreadGraph._unspreadingHandler(spreadLink, context, function (permission) {
              if (permission) {
                _this8.spreadGraph.remove(spreadLink.id, function (error, count) {
    ...
    value: function spreadTo(id, context, handler, callback) {
      ...
      var queue = _async2.default.queue(function (pathGraph, nextPathGraph) {
        pathGraph.fetch(_defineProperty({}, pathGraph.toField, id), undefined, function (error, pathLinks) {
          _async2.default.each(pathLinks, function (pathLink, nextPathLink) {
            _this9.spreadByPathLink(pathGraph, pathLink, context, handler, nextPathLink);
          }, function (error) {
            nextPathGraph();
          });
        });
